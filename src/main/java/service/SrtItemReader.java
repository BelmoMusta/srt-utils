...
public class SrtItemReader {
	...
	public static Set<SrtItem> createItemsFromContent(String content, String instant) {
		Set<SrtItem> itemsFromContent = createItemsFromContent(content);
        for (SrtItem srtItem : itemsFromContent) {
            srtItem.delay(instant);
        }
		return itemsFromContent;
	}
